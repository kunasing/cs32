...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    for (int i = 0; i < vStudents.size(); i++)
        students[i] = *vStudents[i];
    
//    vector<Student>::iterator p = students.begin();
//    for (vector<Student*>::iterator it = vStudents.begin(); it != vStudents.end(); it++, p++)
//        *p = *(*it);
