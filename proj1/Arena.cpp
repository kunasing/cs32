...
void Arena::setCellStatus(int r, int c, int status)
{
    ...
    //history function, check if not already carrot placed. if not, then record in history
    if (m_grid[r-1][c-1] != HAS_POISON && status == HAS_POISON)
        gameHistory.record(r, c);
